...
import { Link, UrlsListResponse, UrlsResponse } from '../models/urls';
...
export class UrlsService {
    ...
    postUrl(data: Link): Observable<Link> {
        return this.http.post<Link>(this.apiEndpoint, JSON.stringify(data));
    }
    ...
    putUrl(data: Link): Observable<Link> {
        return this.http.put<Link>(this.apiEndpoint + data.id + '/', JSON.stringify(data));
    }
    ...
    getAllUrls(): Observable<Link[]> {
        return this.http.get<{ data: Link[] }>(this.apiEndpoint)
            .pipe(map(response => {
